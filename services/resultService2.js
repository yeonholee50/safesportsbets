...
mongoose.connect(
  ...
  const updateResults = async () => {
    ...
    const getSerieResults = async () => {
      await sleep(1000);
      console.log('Slept for 1 second before Serie results API call');
      return axios.get(
                ...
                },
                {
                  ...
                  if (doc != null) {
                    if (doc.status === 'Completed') {
                      resultsArr.push({"gameUID": doc.gameUID,  "results": doc.game.results.final, "full_results": doc.game.results.full})
                    }
                  }
                }
              )
                ...
                await Slip.findOneAndUpdate(
                  { "_id": doc._id },
                  {
                    $set: {
                      ...
                      [`slips.keys.${ betUID }.game`]: results.full_results
                    },
